...
parser_error_t _toStringbool(const pd_bool_t *v, char *outValue, uint16_t outValueLen, __Z_UNUSED uint8_t pageIdx,
                             uint8_t *pageCount) {
    CLEAN_AND_CHECK()
...
parser_error_t _toStringCompactBalanceOf(const pd_CompactBalanceOf_t *v, char *outValue, uint16_t outValueLen,
                                         uint8_t pageIdx, uint8_t *pageCount) {
    CHECK_ERROR(_toStringCompactInt(&v->value, COIN_AMOUNT_DECIMAL_PLACES, false, "", COIN_TICKER, outValue,
