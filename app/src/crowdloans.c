...
static const char *crowdloan_message_01 =
...
zxerr_t account_getItem(int8_t displayIdx, char *outKey, uint16_t outKeyLen, char *outVal, uint16_t outValLen,
    ...
    char tmpBuf[250];
    snprintf(tmpBuf, sizeof(tmpBuf), "%s %s %s %s, %s keys.", (char *)PIC(crowdloan_message_00), account_str,
             (char *)PIC(crowdloan_message_01), (char *)PIC(crowdloan_message_02), chain_str);
